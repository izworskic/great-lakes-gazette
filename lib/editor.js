...
import { buildLedgerEdition } from './ledger-edition.js';
...
// The publication path. The free-writing narrative loop was removed on
// 2026-09-26: its claims could only be graded by another model, never checked,
// and it failed every day from at least Sep 20 while a fallback printed report
// times as arrival estimates. Every edition is now rendered from the fact ledger
// and mechanically verified; JEV may only choose which verified item leads.
export async function produceEdition({ data, issueNumber, recentEditions = [], log = [], publicationDate = michiganDateKey(), now = new Date(), decide }) {
  const brief = await buildLedgerEdition(data, {
    issueNumber, recentEditions, log, publicationDate, now, ...(decide ? { decide } : {}),
  });
  return { brief, report: { mode: 'fact-ledger', total: null }, bannedSubjects: [] };
}
