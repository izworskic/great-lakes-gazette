...
import { buildSourceBulletin } from './source-bulletin.js';
...
export const SCORING_MATRIX = {
  ...
  structure:       { weight: 10, desc: 'Scannable: lead then departments, varied section lengths, length proportional to available facts, nothing repeated between sections.' },
...
export function mechanicalChecks(ed, bannedSubjects = []) {
  ...
  if (/\bsomething\b/i.test(all)) {
    mustFix.push('Contains placeholder wording using "something". Replace it with the exact sourced fact or remove the claim.');
  }
  const words = String(ed.brief || '').split(/\s+/).filter(Boolean).length;
  if (!words) mustFix.push("Body is empty.");
...
- Reject unsupported draft/clearance/cargo conclusions from lake-datum readings, invented geographic distances, ETAs presented as completed or certain arrivals, and missing reports described as quiet waterways. Each is a blocking mustFix, regardless of total score.
...
export async function scoreEdition(ed, {
  dataContext,
  recentEditions = [],
  bannedSubjects = [],
  publicationDate = michiganDateKey(),
  client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY, timeout: 45000, maxRetries: 0 }),
}) {
  // The critic must see EXACTLY the evidence supplied to the writer. Truncating
  // at 9,000 characters hid later AIS ports, news, NOAA and NWS observations and
  // caused valid facts to be rejected as inventions on every scheduled retry.
  if (!dataContext?.trim()) throw new Error('Cannot fact-check an edition without its source context');
  const recentBlock = recentEditions.length
    ? recentEditions.map(e => `${e.date}: "${e.headline}" [lead: ${e.leadSubject || 'unknown'}]`).join('\n')
    : 'none on file';
  const user = `Today is ${formatPublicationDate(publicationDate, { weekday: true })}.
...
export function assertEditorialReady(report) {
  if (!report || report.total < 90 || report.scores?.grounding < 12 || !Array.isArray(report.mustFix) || report.mustFix.length) {
    throw new Error('Editorial gate failed; retain the latest published edition rather than publish an unresolved draft');
  }
}

async function produceNarrativeEdition({
  data,
  issueNumber,
  recentEditions,
  log = [],
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    // Leave time within the 300-second function budget for the source bulletin,
    // Redis publication and verification if narrative revisions do not converge.
    if (attempt > 1 && Date.now() - startedAt > 150000) break;
    const ed = await generateBrief(data, {
      issueNumber,
      recentEditions,
      bannedSubjects,
      critique,
    ...
    const dateProblems = validateEditionDateIntegrity(ed, publicationDate);
    if (dateProblems.length) {
      const scores = Object.fromEntries(Object.keys(SCORING_MATRIX).map(key => [key, 0]));
      const report = {
        total: 0,
        scores,
        mustFix: dateProblems.map(problem => `Calendar integrity: ${problem}`),
        notes: ['Use the calendar date as authoritative; correct or remove the weekday.'],
      };
      log.push(`[${stamp()}] Editorial loop attempt ${attempt}: deterministic date gate failed`);
      log.push(`[${stamp()}]   mustFix: ${report.mustFix.join(' | ').slice(0, 300)}`);
      critique = report;
      priorDraft = ed;
      continue;
    }
    const report = await scoreEdition(ed, {
      dataContext: ed._context,
      recentEditions,
      bannedSubjects,
      publicationDate,
    });
    ...
    try {
      assertEditorialReady(report);
      best = { ed, report, attempt };
    } catch { /* Continue revisions; never publish an unresolved draft. */ }
    if (best && report.total >= SHIP_AT && report.mustFix.length === 0) break;
    critique = report;
    priorDraft = ed;
  }

  if (!best) {
    throw new Error(`Writer failed the publication integrity gates after ${MAX_ATTEMPTS} attempts`);
  }
  const { ed, report, attempt } = best;
  assertEditorialReady(report);
  assertEditionDateIntegrity(ed, publicationDate);
  delete ed._context;
...
export async function produceEdition(options) {
  try {
    return await produceNarrativeEdition(options);
  } catch (error) {
    // Discard every rejected draft. This format copies dated source fields,
    // keeps the source-health gates and carries no invented editorial score.
    options.log?.push(`Narrative unavailable: ${error.message}; preparing a checked source bulletin`);
    const brief = buildSourceBulletin(options.data, { ...options, reason: 'Narrative generation or editorial checks failed' });
    return { brief, report: { mode: 'source-bulletin', total: null }, bannedSubjects: [] };
  }
}
