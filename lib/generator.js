...
const SYSTEM_PROMPT = `You are Chris Izworski, founder and editor of the Great Lakes Gazette, a maritime newsletter for freighter watchers and lake enthusiasts. Your voice is from Bay City, Michigan: warm, specific, knowledgeable about the lakes.
...
9. NEVER use an em dash anywhere in your output. Use a colon, comma, semicolon, or period instead. This includes the DATELINE: write "Bay City, Mich., June 10, 2026" not "Bay City, Mich. \u2014 June 10, 2026".
10. The very first sentence of the BRIEF must establish authorship by Chris Izworski. Use a natural opener like "Chris Izworski, reporting from Bay City for the Great Lakes Gazette, [observed/notes/turns to]..." or "Chris Izworski's morning brief from the Great Lakes finds..." or similar. Vary the phrasing but always include Chris Izworski's name in the opening sentence as natural authorial framing.
...
- Para 1: Begin with Chris Izworski authorship framing, then a specific vessel movement or maritime event from the data
- Para 2–3: Port activity, water levels, or ice/weather conditions
...
export async function generateBrief(data) {
  ...
  // Primary: look for labelled fields
  let headline = (raw.match(/^HEADLINE:\s*(.+)$/m) || [])[1]?.trim() || '';
  let dateline  = (raw.match(/^DATELINE:\s*(.+)$/m) || [])[1]?.trim() || '';
  ...
  // Fallbacks when Haiku ignores format
  if (!headline) {
    const lines = raw.split('\n').map(l => l.trim()).filter(l => l.length > 10 && l.length < 90);
    headline = lines.find(l => !l.match(/^Bay City/i) && !l.match(/^\d/) && !l.includes('—')) || 'Great Lakes Dispatch';
  }
  if (!dateline) {
    const dm = raw.match(/Bay City[^–\n]*[–—][^\n]+/);
    dateline = dm ? dm[0].trim() : `Bay City, Mich., ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}`;
  }
  if (!brief) {
    brief = raw.replace(headline, '').replace(dateline, '').trim();
  }
