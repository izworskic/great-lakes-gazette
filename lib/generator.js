...
import { formatPublicationDate, michiganDateKey } from './dates.js';
...
export function isFreshSourceDate(value, now = Date.now(), maxAgeHours = 48) {
...
function formatAISSection(aisPassages) {
  ...
  let anyData = false;
  let anyHealthyPort = false;
  for (const port of aisPassages) {
    if (port.status === 'ok') anyHealthyPort = true;
    if (port.status === 'ok' && port.vessels.length > 0) {
      ...
      for (const v of port.vessels) {
        ...
        const place = v.location    ? ` | Location: ${v.location}` : '';
        const dest  = v.destination ? ` → ${v.destination}` : '';
        const eta   = v.eta         ? ` | ETA: ${v.eta}` : '';
        const cargo = v.cargo       ? ` | Cargo: ${v.cargo}` : '';
        const seen  = v.timestamp   ? ` | Last report: ${v.timestamp}` : '';
        lines.push(`  • ${v.name}${dir}${place}${dest}${eta}${cargo}${seen}`);
      }
    }
  }
  if (!anyData && anyHealthyPort) lines.push('\nAIS source is online but no named passages were returned. Do NOT invent vessel positions.');
  if (!anyData && !anyHealthyPort) lines.push('\nAIS source is unavailable. Do NOT claim the navigation season is closed and do NOT invent vessel positions.');
  return lines.join('\n');
}

function formatPortReports(portReports, now) {
  const lines = ['=== PORT REPORTS ==='];
  for (const r of portReports) {
    if (!/^port report/i.test(r.title)) continue;
    if (!isFreshSourceDate(r.date, now)) continue;
...
function formatVesselNews(shippingNews, now) {
  const lines = ['=== VESSEL MOVEMENTS & MARITIME NEWS ==='];
  for (const post of (shippingNews || [])) {
    if (!isFreshSourceDate(post.date, now)) continue;
...
function formatWaterLevels(waterLevels) {
  ...
  for (const w of good) {
    lines.push(`${w.lake}: ${w.level_ft.toFixed(2)} ft above Low Water Datum (${w.city}; NOAA station ${w.stationId}; observed ${w.date} local standard time)`);
  }
  return lines.join('\n');
}
...
export function buildContext(data, now = Date.now()) {
  return [
    formatAISSection(data.aisPassages     || []),
    formatPortReports(data.portReports    || [], now),
    formatVesselNews(data.shippingNews    || [], now),
...
2. FRESHNESS: every data item carries a date. A movement more than 48 hours old is background context, never the lead and never framed as today's news. If all vessel data is stale, lead with what IS current: water levels, marine weather, or the history item, and state that current vessel reports are unavailable. Missing reports never establish quiet or empty waterways.
...
13. TOMORROW'S WATCH names a source the reader can check. Preserve uncertainty: an AIS ETA is an estimate, not a confirmed arrival, passage, queue position or sailing plan. Never promise an outcome the data does not establish.
15. WATER LEVELS: report the named NOAA station, observation time and local Low Water Datum. A station reading is not a lake-wide mean. Do not infer channel depth, vessel clearance, cargo capacity, loading constraints or beach access from these readings. Comparing levels above different lake datums cannot rank navigability. Do not invent distances or relationships between vessels.
14. CALENDAR INTEGRITY: a weekday must agree with the calendar date beside it. The publication date in the user message is authoritative. If source material gives a date but not a weekday, do not invent the weekday unless you can compute it correctly.
...
- "The Levels Ledger": a dated station-by-station NOAA snapshot, with local datum and limits, 40 to 100 words. Only when levels are in the data.
...
Aim for 380 to 620 words when supported. A shorter fact-rich edition is preferable to padding. No fact appears in two sections.
...
export async function generateBrief(data, opts = {}) {
  ...
  const today = formatPublicationDate(publicationDate, { weekday: true });
...
Rewrite the edition. Address every item under Must fix one by one; where a claim cannot be verified against the data, delete the claim instead of adjusting it. Raise the weakest categories and keep what the critique did not fault.`;
  ...
  const portReportUrls = (data.portReports || []).filter(p => p.url && /^port report/i.test(p.title) && isFreshSourceDate(p.date, now))
                                                 .map(p => ({ title: p.title, url: p.url, date: p.date }));

  return {
    headline:    scrub(parsed.headline) || 'Great Lakes Dispatch',
    deck:        scrub(parsed.deck),
    dateline:    scrub(parsed.dateline) || `Bay City, Mich., ${issueDate}`,
