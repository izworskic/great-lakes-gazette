...
.topic-desc{display:block;font-size:13.5px;line-height:1.5;color:var(--ink-3);margin-top:6px}
...
export function topicDirectoryHtml({ counts = {}, heading = 'Explore Great Lakes Shipping Topics' } = {}) {
  ...
  <div class="topic-grid">
    ${TOPICS.map(topic => `<a class="topic-card" href="${topicUrl(topic)}">
      ...
      <span class="topic-desc">${esc(topic.description)}</span>
      ${Number.isFinite(counts[topic.slug]) ? `<span class="topic-count">${counts[topic.slug]} matching edition${counts[topic.slug] === 1 ? '' : 's'}</span>` : ''}
    </a>`).join('\n    ')}
  </div>
</section>`;
}
