...
/* Durable topic directory and hub cards */
.topic-directory{margin-top:52px;border-top:3px double var(--ink);padding-top:26px}
.topic-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}
.topic-card{display:block;border:1px solid var(--ink-4);background:rgba(255,255,255,.28);padding:16px;color:var(--ink)}
.topic-card:hover{border-color:var(--lake);text-decoration:none;background:rgba(27,58,86,.05)}
.topic-name{font-family:'IM Fell English',Georgia,serif;font-size:19px;line-height:1.2;color:var(--ink)}
.topic-latest{display:block;font-size:13.5px;line-height:1.45;color:var(--ink-3);margin-top:7px}
.topic-count{display:block;font-family:'Josefin Sans',sans-serif;font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:var(--lake);margin-top:9px}
...
export function topicDirectoryHtml({ counts = {}, latestByTopic = {}, heading = 'Shipping Beats' } = {}) {
  return `<section class="topic-directory">
  <div class="section-label">${esc(heading)}</div>
  <div class="topic-grid">
    ${TOPICS.map(topic => `<a class="topic-card" href="${topicUrl(topic)}">
      <span class="topic-name">${esc(topic.name)}</span>
      ${latestByTopic[topic.slug] ? `<span class="topic-latest">Latest: ${esc(latestByTopic[topic.slug].headline)}</span>` : ''}
      ${Number.isFinite(counts[topic.slug]) ? `<span class="topic-count">${counts[topic.slug]} edition${counts[topic.slug] === 1 ? '' : 's'}</span>` : ''}
    </a>`).join('\n    ')}
  </div>
</section>`;
}
...
export function footerHtml() {
  ...
  <div class="f-line"><a href="/">Today's Edition</a> · <a href="/topics">Shipping Topics</a> · <a href="/archive">Archive</a> · <a href="/feed.xml">RSS</a> · <a href="/feed.json">JSON Feed</a> · <a href="/chris-izworski">About the Author</a></div>
