...
import { TOPICS, topicUrl } from './topics.js';
...
export const FAVICON = `<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ctext y='.9em' font-size='90'%3E⚓%3C/text%3E%3C/svg%3E">`;

export const FEED_LINK = `<link rel="alternate" type="application/rss+xml" title="Great Lakes Gazette RSS" href="${SITE}/feed.xml">
<link rel="alternate" type="application/feed+json" title="Great Lakes Gazette JSON Feed" href="${SITE}/feed.json">`;
...
/* Compact newspaper section navigation */
.section-strip{margin:20px 0 10px;padding:13px 0 7px;border-top:1px solid var(--ink-4);border-bottom:1px solid var(--ink-4)}
.section-strip .section-label{margin-bottom:8px}
.topic-pills{display:flex;gap:8px;flex-wrap:wrap;margin:18px 0 24px}
.topic-pill{display:inline-block;border:1px solid var(--ink-4);padding:5px 9px;font-family:'Josefin Sans',sans-serif;font-size:10px;letter-spacing:.09em;text-transform:uppercase;color:var(--lake)}
.topic-pill:hover{border-color:var(--lake);text-decoration:none}
.topic-count{color:var(--ink-3);letter-spacing:0}
.edition-summary{font-size:14.5px;color:var(--ink-3);line-height:1.55;margin-top:5px}
.news-lead{margin-top:24px;border-top:3px double var(--ink);padding-top:8px}
.news-lead .r-item:first-child .r-head{font-size:25px}
...
export function followBox() {
  ...
  <p style="font-size:12.5px;color:var(--ink-3);margin-bottom:0">Full-text <a href="/feed.xml">RSS</a> and <a href="/feed.json">JSON Feed</a>. Works with Feedly, NetNewsWire, Inoreader, or any reader. Also: <a href="https://www.youtube.com/@izworskic" rel="me">YouTube</a> and <a href="/chris-izworski">the author page</a>.</p>
</div>`;
}

export function topicSectionNavHtml({ counts = {}, heading = 'Browse by Waterway' } = {}) {
  return `<nav class="section-strip" aria-label="Great Lakes shipping sections">
  <div class="section-label">${esc(heading)}</div>
  <div class="topic-pills">
    ${TOPICS.map(topic => `<a class="topic-pill" href="${topicUrl(topic)}">${esc(topic.name)}${Number.isFinite(counts[topic.slug]) ? ` <span class="topic-count">(${counts[topic.slug]})</span>` : ''}</a>`).join('\n    ')}
  </div>
</nav>`;
}
...
export function aboutStrip() {
  return `<div class="about-strip">The Great Lakes Gazette assembles a daily brief from vessel tracking (AIS) relayed by BoatNerd, NOAA water levels and National Weather Service marine forecasts. Since September 26, 2026, every sentence is generated from those source fields and checked against them before publication; an AI model only chooses which verified item leads and writes none of the text. Earlier editions were AI-written from the same sources. Automated checks do not replace a personal review of each edition. Published by <a href="/chris-izworski">${AUTHOR}</a> in Bay City, Michigan, where the Saginaw River meets the bay.</div>`;
}

export function publishingNote(brief) {
  if (brief?.editorial?.mode === 'fact-ledger') {
    const lead = brief.editorial.lead?.chosenBy === 'jev' ? 'An AI model chose which verified item leads; it wrote none of the text.' : 'The lead was chosen by fixed rules.';
    return `<p class="publishing-note" style="font-size:13px;color:var(--ink-2);line-height:1.6"><strong>How this edition was made:</strong> Every sentence is generated from a dated BoatNerd, NOAA or NWS source field and checked against those stored facts before publication. ${lead} Estimates are marked as estimates and positions change after their report time. Verify time-sensitive details with the original provider. <a href="https://chrisizworski.com/connect/">Report a correction</a>.</p>`;
  }
  if (brief?.editorial?.mode === 'source-bulletin') return `<p class="publishing-note" style="font-size:13px;color:var(--ink-2);line-height:1.6"><strong>Source bulletin:</strong> Automatically assembled from dated source fields because the narrative edition was unavailable. No rejected AI draft is included. Report times and estimates are preserved; verify travel decisions with the original provider. <a href="https://chrisizworski.com/connect/">Report a correction</a>.</p>`;
  return `${brief?.correctionNote ? `<p class="correction-note"><strong>Correction:</strong> ${esc(brief.correctionNote)} <a href="https://www.glerl.noaa.gov/data/wlevels/">NOAA measurement context</a>.</p>` : ''}<p class="publishing-note" style="font-size:13px;color:var(--ink-2);line-height:1.6">AI-generated briefing from linked source data, with automated editorial checks. Source feeds and summaries can be incomplete or mistaken. Verify time-sensitive details with the original provider. <a href="https://chrisizworski.com/connect/">Report a correction</a>.</p>`;
}

export function footerHtml() {
  return `<footer class="site-footer">
  <div>Great Lakes Gazette · Published by <a href="${AUTHOR_URL}">${AUTHOR}</a> in Bay City, Michigan · Founded 2026</div>
  <div class="f-line"><a href="/">Today's Edition</a> · <a href="/topics">Shipping Sections</a> · <a href="/archive">Archive</a> · <a href="/feed.xml">RSS</a> · <a href="/feed.json">JSON Feed</a> · <a href="/chris-izworski">About the Author</a></div>
  ...
  <div class="f-line"><a href="https://chrisizworski.com/privacy/">Privacy Policy</a> · <a href="https://chrisizworski.com/terms/">Terms of Use</a> · <a href="https://chrisizworski.com/connect/">Contact and corrections</a></div>
...
// Department-aware article body. New editions carry sections[] with kickers
// and a Tomorrow's Watch tease; old editions fall back to the flat brief.
export const DEPT_CSS = `
.deck{font-style:italic;font-size:1.05rem;line-height:1.5;color:#3d3a34;margin:.35rem 0 .9rem}
.dept-kicker{font-variant:small-caps;letter-spacing:.14em;font-weight:700;font-size:.78rem;color:#6b2d16;border-top:1px solid #d8d2c4;padding-top:.75rem;margin:1.15rem 0 .35rem}
.tomorrow{border:1px solid #d8d2c4;background:#f6f2e8;padding:.6rem .8rem;margin-top:1.1rem;font-size:.92rem}
.tomorrow b{font-variant:small-caps;letter-spacing:.12em;font-size:.78rem;color:#6b2d16;display:block;margin-bottom:.15rem}
`;

export function articleBodyHtml(brief) {
  const secs = Array.isArray(brief && brief.sections) ? brief.sections.filter(s => s && s.body) : [];
  if (!secs.length) return paragraphs(brief && (brief.brief || ''));
  let html = '';
  for (const s of secs) {
    if (s.kicker) html += `\n<div class="dept-kicker">${esc(stripDashes(s.kicker))}</div>`;
    html += `\n${paragraphs(s.body)}`;
  }
  if (brief.tomorrow) {
    html += `\n<div class="tomorrow"><b>Tomorrow's Watch</b> ${esc(stripDashes(brief.tomorrow))}</div>`;
  }
  return html;
}
