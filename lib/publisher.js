...
export function buildPostHtml(briefData) {
  const { brief, deck, sections, tomorrow, issueDate, issueNumber, sources } = briefData;
...
${aisNote}This briefing is AI-generated from source data with automated checks. Verify important details with the linked original providers.
</p>`;
  ...
  let core;
  if (secs.length) {
    core = (deck ? `<p><em>${deck}</em></p>\n` : '') +
      secs.map(s => (s.kicker ? `<h3>${s.kicker}</h3>\n` : '') + paras(s.body)).join('\n') +
      (tomorrow ? `\n<h3>Tomorrow's Watch</h3>\n<p>${tomorrow}</p>` : '');
  } else {
    core = paras(brief);
  }
  return { title, bodyHtml: core + footer };
}

export async function updateWordPressPost(postId, briefData) {
  if (!WP_TOKEN) throw new Error('WP_TOKEN environment variable is not set');
  const { title, bodyHtml } = buildPostHtml(briefData);
  const r = await fetch(`https://public-api.wordpress.com/rest/v1.1/sites/${WP_SITE_ID}/posts/${postId}`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${WP_TOKEN}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ title, content: bodyHtml }),
  });
  if (!r.ok) throw new Error(`WP update failed ${r.status}: ${(await r.text()).slice(0, 300)}`);
  return r.json();
}
