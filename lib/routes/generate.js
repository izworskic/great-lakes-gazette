...
// GET /api/generate
// Cache strategy:
//   1. Upstash Redis (UPSTASH_REDIS_REST_URL + TOKEN) - persistent, keyed by Michigan date
...
import { produceEdition } from '../editor.js';
import { parseIssue, saveIssue, getDates, getIssues, INDEX_KEY } from '../store.js';
...
export default async function handler(req, res) {
  ...
  try {
    if (!forceRefresh) {
      ...
      if (cached) {
        const parsed = parseIssue(cached, today);
    ...
    // 3. Cache miss. Generation costs an Anthropic call, so only
    // authorized callers may trigger it; the public gets a friendly wait.
    if (req.headers['authorization'] !== `Bearer ${process.env.CRON_SECRET}`) {
      res.setHeader('Cache-Control', 'no-store');
      return res.status(425).json({ success: false, error: "Today's edition is not yet published. A new brief arrives every morning; read the latest at https://gazette.chrisizworski.com/." });
    }
    // Cache miss, authorized: scrape + generate (one Anthropic call)
    console.log('[generate] MISS - generating for', today, r ? '(Redis available)' : '(no Redis)');
    const data = await fetchAllData();

    let brief = null;
    try {
      let issueNumber = 0, recentEditions = [];
      if (r) {
        const already = await r.sismember(INDEX_KEY, today);
        const count   = await r.scard(INDEX_KEY);
        issueNumber   = already ? count : count + 1;
        const dts = (await getDates(r)).filter(d => d !== today).slice(0, 7);
        const map = await getIssues(r, dts);
        recentEditions = dts.map(d => {
          const it = map.get(d);
      ...
      const loopLog = [];
      const out = await produceEdition({
        data,
        issueNumber,
        recentEditions,
        log: loopLog,
      ...
      brief = out.brief;
      for (const l of loopLog) console.log('[generate]', l);
      console.log(`[generate] Edition accepted at ${out.report.total}/100: ${brief.headline}`);
    } catch(e) {
      console.error('[generate] Edition failed:', e.message);
      throw e;
    }

    const payload = { data, brief, generated_at: new Date().toISOString() };
    assertPublishableIssue(payload, today);

    // 4. Persist to Redis (survives cold starts until midnight)
    if (r) { await saveIssue(r, today, payload); console.log('[generate] Saved permanently; gazette:index updated'); }
