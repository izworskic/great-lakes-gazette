...
// history, wire headlines, recent editions. Content never depends on client
// JavaScript; the only browser script is optional first-party pageview measurement.
...
import {
  ...
  headCommon, css, mastheadHome, followBox, resourcesBox, aboutStrip, footerHtml, publishingNote,
} from '../layout.js';
...
function marineWidget(items, editionDate) {
  ...
  <div class="w-note">NWS marine synopses from the ${esc(editionDate)} edition</div>
</div>`;
}
...
export function renderHome({ dates, issuesMap }) {
  ...
  // Feature the newest date whose payload actually parsed to a brief, so a
  // stray index entry with no stored issue can never blank the front page.
  let today = null, issue = null;
  for (const d of dates) {
    const it = issuesMap.get(d);
    if (it && it.brief && it.brief.brief) { today = d; issue = it; break; }
  }
  if (!today) today = dates[0] || null;
  const brief   = (issue && issue.brief) || null;
  const data    = (issue && issue.data) || {};
  const currentMichiganDate = michiganDateKey();
  const dateStr = today ? longDate(today) : longDate(currentMichiganDate);

  const graph = JSON.parse(JSON.stringify(HOME_GRAPH));
  for (const node of (graph['@graph'] || [])) {
    if (node['@type'] === 'WebPage' || node['@type'] === 'Dataset') {
      node.dateModified = today || currentMichiganDate;
    }
  }
  ...
  if (brief) {
    ...
    article = `<article>
  ${today !== currentMichiganDate ? `<p role="status">Today's edition is delayed. Below is the latest published edition, from ${esc(dateStr)}. For current vessel reports, open <a href="https://chrisizworski.com/soo-locks/">Soo Locks Today</a> or the <a href="https://chrisizworski.com/great-lakes-freighter-tracking/">live ship tracker</a>.</p>` : ''}
  <div class="kicker">${esc(dateline)}</div>
  <h2 class="headline"><a href="/issue/${today}">${esc(headline)}</a></h2>
  <div class="byline">Published by <a href="/chris-izworski">${AUTHOR}</a> &nbsp;&middot;&nbsp; Founder, Great Lakes Gazette &nbsp;&middot;&nbsp; ${esc(shortDate(today))}</div>
  ${publishingNote(brief)}
  ${brief.deck ? `<div class="deck">${esc(stripDashes(brief.deck))}</div>` : ''}
  <div class="brief dropcap">
${articleBodyHtml(brief)}
  </div>
  ...
  <div class="pn"><a href="/issue/${today}">Permanent link to this edition</a><span class="pn-spacer"></span>${prev ? `<a href="/issue/${prev}">Previous edition</a>` : ''}</div>
</article>`;
  } else {
    article = `<article>
  ...
  const recent = dates.filter(d => d !== today).slice(0, 14).map(d => {
    const it = issuesMap.get(d);
...
<div class="wrap">
  <div class="front">
    ...
    <aside class="rail">
      ${followBox()}
      ${marineWidget(data.marineWeather, dateStr)}
