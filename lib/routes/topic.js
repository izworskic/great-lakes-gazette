...
import {
  ...
  headCommon, css, headerInterior, footerHtml, topicSectionNavHtml,
} from '../layout.js';
import {
...
<main class="wrap">
  <div class="breadcrumb"><a href="/">Great Lakes Gazette</a> &rsaquo; Shipping Sections</div>
  <div class="kicker">The Latest from the Five Lakes</div>
  <h1 class="headline">Great Lakes Shipping News</h1>
  ${topicSectionNavHtml({ counts, heading: 'Browse by Waterway' })}
  ...
  const graph = {
    ...
    '@graph': [
      ...
      personSchema(),
      {
        ...
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Home', item: SITE },
          { '@type': 'ListItem', position: 2, name: 'Shipping Sections', item: `${SITE}/topics` },
...
<main class="wrap-narrow">
  <div class="breadcrumb"><a href="/">Great Lakes Gazette</a> &rsaquo; <a href="/topics">Shipping Sections</a> &rsaquo; ${esc(topic.name)}</div>
  ...
  <section class="news-lead" aria-labelledby="topic-editions">
    ...
    ${rows || '<p class="intro">No Gazette lead stories are filed under this section yet.</p>'}
  </section>
  <div class="section-label" style="margin-top:34px">More Shipping Sections</div>
...
function notFoundPage() {
  return `<!DOCTYPE html><html lang="en"><head>${headCommon()}<title>Section not found | Great Lakes Gazette</title><meta name="robots" content="noindex"><style>${css()}</style></head><body>${headerInterior('/topics')}<main class="wrap-narrow" style="padding-top:40px"><h1 class="headline">Section not found</h1><p class="intro">Browse the <a href="/topics">Great Lakes shipping sections</a> or return to <a href="/">today's edition</a>.</p></main>${footerHtml()}</body></html>`;
}

function unavailablePage() {
  return `<!DOCTYPE html><html lang="en"><head>${headCommon()}<title>Shipping sections temporarily unavailable | Great Lakes Gazette</title><meta name="robots" content="noindex"><style>${css()}</style></head><body>${headerInterior('/topics')}<main class="wrap-narrow" style="padding-top:40px"><h1 class="headline">Shipping sections are temporarily unavailable</h1><p class="intro">Today's full edition remains on the <a href="/">Gazette front page</a>. Please try the sections again shortly.</p></main>${footerHtml()}</body></html>`;
}
