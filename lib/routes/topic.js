...
<main class="wrap">
  ...
  <div class="kicker">Continuously Updated Maritime Archives</div>
  <h1 class="headline">Great Lakes Shipping Topics</h1>
  <p class="intro">Explore ${TOPICS.length} focused archives built from the Gazette's daily reporting on freighters, ports, locks, all five lakes, water levels, and marine weather. New editions are filed automatically as they publish.</p>
  ${topicDirectoryHtml({ counts, heading: `${totalMatches} Filed Edition Links Across Eight Topics` })}
  <div class="about-strip">Every topic page is part of the <a href="/archive">permanent Great Lakes Gazette archive</a>. Reports are written and edited by <a href="/chris-izworski">${AUTHOR}</a> from live vessel observations, port reports, NOAA water levels, and National Weather Service marine forecasts.</div>
</main>
${footerHtml()}
</body></html>`;
}
...
<main class="wrap-narrow">
  ...
  <div class="kicker">Daily Reporting by <a href="/chris-izworski">${AUTHOR}</a></div>
  <h1 class="headline">${esc(topic.title)}</h1>
  ${topic.intro.map(paragraph => `<p class="intro">${esc(paragraph)}</p>`).join('\n  ')}
  <div class="byline">By <a href="/chris-izworski">${AUTHOR}</a> &nbsp;&middot;&nbsp; ${items.length} matching edition${items.length === 1 ? '' : 's'} &nbsp;&middot;&nbsp; Updated with each daily Gazette</div>
  <section class="recent" aria-labelledby="topic-editions">
    <h2 class="section-label" id="topic-editions">Latest ${esc(topic.name)} Editions</h2>
    ${rows || '<p class="intro">No matching editions are filed yet. The next relevant daily report will appear here automatically.</p>'}
  </section>
  <nav class="topic-pills" aria-label="More shipping topics">${topicLinks}</nav>
  <div class="about-strip">These reports remain available in the <a href="/archive">complete Gazette archive</a>. The topic index updates from each edition's published reporting, so it adds useful discovery paths without duplicating the newspaper's articles.</div>
</main>
${footerHtml()}
</body></html>`;
}
