...
async function fetchOneAISPort(port) {
  try {
    ...
    // BoatNerd returns HTML when AIS is unavailable (off-season or after site redesign)
    if (text.trimStart().startsWith('<')) return { port: port.name, portId: port.id, vessels: [], status: 'HTTP 500' };
    let data;
    try { data = JSON.parse(text); } catch(e) { return { port: port.name, portId: port.id, vessels: [], status: 'HTTP 500' }; }
