...
const VESSEL_SIGNALS = [
  ...
  /\b(steamer|freighter|laker|tanker|self.unloader|bulker|tug|carferry|saltie|salty|cutter|icebreaker)\b/i,
  /\b(USCGC|CCGS|M\/V|SS |MV )\b/,
  /\b(season|cargo|cement|coal|limestone|grain|iron ore|stone|salt)\b/i,
];
...
function isVesselStory(headline, body) {
  ...
  // Keep if it has vessel signals and isn't dominated by meta noise
  return vesselHits >= 1 && noiseHits <= 1;
}
...
export async function scrapeShippingNews() {
  try {
    ...
    for (const p of posts) {
      ...
      // Split HTML on <h1> or <h2> tags — each is a sub-story headline
      const storyBlocks = html.split(/(?=<h[12][^>]*>)/i).filter(b => b.trim());

      const stories = [];

      for (const block of storyBlocks) {
        // Extract headline
        const headlineMatch = block.match(/<h[12][^>]*>([\s\S]*?)<\/h[12]>/i);
        const headline = headlineMatch ? stripHtmlKeepText(headlineMatch[1]) : '';

        // Extract body text (everything after the headline tag)
        ...
        if (isVesselStory(headline, body)) {
          stories.push({ headline, body });
        }
      }

      // If no h1 splits found (flat post like port reports), treat whole text as one story
      if (stories.length === 0 && !/boatnerd news/i.test(title)) {
        const fullText = stripHtmlKeepText(html).slice(0, 2000);
        stories.push({ headline: title, body: fullText });
      }
