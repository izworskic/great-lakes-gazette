...
const MARINE_ZONES = [
  { id: 'LSZ162', lake: 'Lake Superior', area: 'Western Superior' },
  { id: 'LMZ421', lake: 'Lake Michigan', area: 'Southern Michigan' },
  { id: 'LHZ321', lake: 'Lake Huron',    area: 'Lake Huron' },
  { id: 'LEZ040', lake: 'Lake Erie',     area: 'Lake Erie Central' },
  { id: 'LOZ044', lake: 'Lake Ontario',  area: 'Lake Ontario' },
];
...
// ── 6. NWS Marine Weather ────────────────────────────────────────────────────
async function fetchOneMarineZone(zone) {
  try {
    const r = await fetchTimeout(`${NWS_API}/zones/forecast/${zone.id}/forecast`, {
    ...
    const waveMatch = detail.match(/waves?\s+(\d+(?:\.\d+)?)\s*(?:to\s+(\d+(?:\.\d+)?))?\s*(?:ft|feet)/i);
    const waveHeight = waveMatch ? (waveMatch[2] ? `${waveMatch[1]}–${waveMatch[2]} ft` : `${waveMatch[1]} ft`) : null;
    return {
      lake: zone.lake, area: zone.area, zoneId: zone.id,
      name:     period.name || '',
    ...
    return { lake: zone.lake, area: zone.area, zoneId: zone.id, shortForecast: '', detail: '', status: 'error', error: e.message };
  }
}

export async function scrapeMarineWeather() {
  return Promise.all(MARINE_ZONES.map(z => fetchOneMarineZone(z)));
}
...
export async function fetchAllData() {
  const [portReports, shippingNews, aisPassages, todayInHistory, waterLevels, marineWeather, greatLakesNow] = await Promise.all([
    scrapePortReports(),
    scrapeShippingNews(),
    scrapeAISPassages(),
  ...
  return { portReports, shippingNews, aisPassages, todayInHistory, waterLevels, marineWeather, greatLakesNow, fetched_at: new Date().toISOString() };
}
