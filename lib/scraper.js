...
// ── 2. Vessel News — extracted from BoatNerd posts, meta-noise filtered ───────
// BoatNerd News posts contain multiple vessel stories mixed with site announcements.
// We split by story block, score each on vessel relevance, discard site meta.

const META_NOISE = [
  /boatnerd\.com/i, /winter layup list/i, /website/i, /donate/i,
  /lecture series/i, /museum/i, /press release/i, /new site/i,
  /AIS vessel map/i, /redesign/i, /mobile app/i, /android/i,
  /boatnerd was started/i, /501\(c\)/i, /board of directors/i,
  /advertising sales/i, /non-profit/i, /launching sunday/i,
  /temporarily unavailable/i, /easier to navigate/i,
];

const VESSEL_SIGNALS = [
  /\b(departed|arrived|departs|arrives|steamed|inbound|outbound|upbound|downbound|unloaded|loaded|berthed|anchored|transited|icebreaking|ice.break)\b/i,
  /\b(steamer|freighter|laker|tanker|self.unloader|bulker|tug|carferry|saltie|salty)\b/i,
  /\b(USCGC|CCGS|M\/V|S\.S\.|SS |MV )\b/,
  /\b[A-Z][a-z]+ [A-Z][a-z]+\b.*\b(departed|arrived|loaded|built|launched)\b/,
  /[A-Z]{2,}.*(?:departed|arrived|inbound|outbound|ARRIVES|DEPARTS)/,
  /\bshe\b.*\b(departed|arrived|built|launched|steamed)\b/i,
];

function scoreVesselRelevance(text) {
  let score = 0;
  for (const re of META_NOISE) { if (re.test(text)) score -= 3; }
  for (const re of VESSEL_SIGNALS) { if (re.test(text)) score += 2; }
  // Bonus for city dateline pattern "CITY, ST –"
  if (/[A-Z]{2,},\s+[A-Z]{2}\s+[–-]/.test(text)) score += 1;
  return score;
}

function extractVesselStories(rawText, sourceUrl) {
  // Split on &nbsp; boundaries (BoatNerd uses these between stories) or double newline
  const blocks = rawText
    .split(/\u00a0+|\n{2,}|(?<=[.!?])\s{2,}(?=[A-Z])/)
    .map(b => b.replace(/https?:\/\/\S+/g, '').replace(/\s+/g, ' ').trim())
    .filter(b => b.length > 60);

  // Group consecutive sentences into story chunks (~150-400 chars each)
  const chunks = [];
  let buf = '';
  ...
  if (buf.length > 60) chunks.push(buf);

  // Score and filter
  return chunks
    .map(c => ({ text: c, score: scoreVesselRelevance(c) }))
    .filter(c => c.score > 0)
    .map(c => c.text);
}
...
export async function scrapeShippingNews() {
  try {
    // Fetch recent posts without the "boatnerd news" search filter — gets more variety
    ...
    for (const p of posts) {
      ...
      // Skip pure history posts — those go in the history widget
      if (/today in great lakes history/i.test(title)) continue;

      const rawText = stripHtml(p.content?.rendered || '');
      const stories = extractVesselStories(rawText, p.link);
      ...
      if (stories.length > 0) {
        vesselItems.push({
          ...
          date:      p.date,
          url:       p.link,
          stories,   // array of vessel-focused text chunks
        });
      }
    }
...
async function fetchOneWaterLevel(station) {
  try {
    const params = new URLSearchParams({
      station: station.id, product: 'water_level', datum: 'LWD',
      time_zone: 'lst_ldt', units: 'english',
      application: 'web_services', format: 'json', date: 'latest',
    });
    ...
    return { lake: station.lake, city: station.city, stationId: station.id, level_ft: parseFloat(obs.v), time: obs.t, status: 'ok' };
  } catch(e) {
...
// ── Master fetch — weather is separate via /api/weather ───────────────────────
export async function fetchAllData() {
  const [portReports, shippingNews, aisPassages, todayInHistory, waterLevels, greatLakesNow] = await Promise.all([
    scrapePortReports(),
    scrapeShippingNews(),
    scrapeAISPassages(),
  ...
  return { portReports, shippingNews, aisPassages, todayInHistory, waterLevels, greatLakesNow, fetched_at: new Date().toISOString() };
}
