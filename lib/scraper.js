...
export async function scrapeShippingNews() {
  try {
    // Get latest general news posts (not port reports)
...
export async function scrapeAISPassages() {
  ...
  for (const port of AIS_PORTS) {
    try {
      ...
      if (!r.ok) {
        results.push({ port: port.name, portId: port.id, vessels: [], status: `HTTP ${r.status}` });
        continue;
      }

      const text = await r.text();
      if (!text || text.trim() === '') {
        results.push({ port: port.name, portId: port.id, vessels: [], status: 'empty_response' });
        continue;
      }

      let data;
      try { data = JSON.parse(text); } catch(e) {
        results.push({ port: port.name, portId: port.id, vessels: [], status: 'parse_error', raw: text.slice(0, 100) });
        continue;
      }

      if (data.error_message) {
        results.push({ port: port.name, portId: port.id, vessels: [], status: data.error_message });
        continue;
      }
      ...
      results.push({ port: port.name, portId: port.id, vessels: [], status: 'error', error: e.message });
    }
  }
  return results;
}
...
// Scrapes the "Today in GL History" posts for extra color
