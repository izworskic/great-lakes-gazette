...
const BOATNERD_AIS_ORIGIN = 'https://ais.boatnerd.com';
...
// One NOAA NWLON station on each lake, plus Essexville for Saginaw Bay. The
// printed station name always comes from NOAA's own response metadata; these
// ids were checked against the CO-OPS station registry on 2026-09-26. (The old
// list printed "Chicago", "Cleveland", "Rochester" and "Sault Ste. Marie" for
// stations NOAA names Holland, Fairport, Oswego and West Neebish Island.)
const WATER_LEVEL_STATIONS = [
  { id: '9099018', lake: 'Lake Superior' },
  { id: '9087031', lake: 'Lake Michigan' },
  { id: '9075014', lake: 'Lake Huron' },
  { id: '9075035', lake: 'Saginaw Bay, Lake Huron' },
  { id: '9063053', lake: 'Lake Erie' },
  { id: '9052030', lake: 'Lake Ontario' },
];
...
let boatNerdClientConfigPromise = null;
...
// BoatNerd's current passage application is a browser client backed by
// /api/v1. Its public client key is shipped in the application's JavaScript
// bundle and can rotate when BoatNerd deploys. Discovering that public config
// at runtime avoids baking a rotating value into this repository and, more
// importantly, turns another frontend migration into a visible source error
// instead of seven misleading HTTP 500 passage results.
export function extractBoatNerdClientConfig(html, bundle, origin = BOATNERD_AIS_ORIGIN) {
  const asset = String(html || '').match(/src=["']([^"']*\/assets\/index\.[^"']+\.js)["']/i)?.[1];
  const apiKey = String(bundle || '').match(/\bAPI_KEY\s*=\s*["']([^"']+)["']/)?.[1];
  const apiPath = String(bundle || '').match(/\bAPI_BASE_URL\s*=\s*["'](\/api\/v\d+)["']/)?.[1];
  if (!asset) throw new Error('BoatNerd client bundle was not found');
  if (!apiKey || !apiPath) throw new Error('BoatNerd public API client config was not found');
  return {
    assetUrl: new URL(asset, origin).toString(),
    apiBaseUrl: new URL(apiPath, origin).toString().replace(/\/$/, ''),
    apiKey,
  };
}

async function discoverBoatNerdClientConfig() {
  const configuredKey = process.env.BOATNERD_AIS_API_KEY;
  if (configuredKey) {
    return { apiBaseUrl: `${BOATNERD_AIS_ORIGIN}/api/v1`, apiKey: configuredKey };
  }

  const home = await fetchTimeout(`${BOATNERD_AIS_ORIGIN}/`, {}, 12000);
  if (!home.ok) throw new Error(`BoatNerd app HTTP ${home.status}`);
  const html = await home.text();
  const asset = html.match(/src=["']([^"']*\/assets\/index\.[^"']+\.js)["']/i)?.[1];
  if (!asset) throw new Error('BoatNerd client bundle was not found');
  const bundleResponse = await fetchTimeout(new URL(asset, BOATNERD_AIS_ORIGIN).toString(), {}, 20000);
  if (!bundleResponse.ok) throw new Error(`BoatNerd bundle HTTP ${bundleResponse.status}`);
  return extractBoatNerdClientConfig(html, await bundleResponse.text());
}

async function getBoatNerdClientConfig() {
  if (!boatNerdClientConfigPromise) {
    boatNerdClientConfigPromise = discoverBoatNerdClientConfig().catch(error => {
      boatNerdClientConfigPromise = null;
      throw error;
    });
  }
  return boatNerdClientConfigPromise;
}
  ...
  try {
    ...
    return posts
      ...
      .filter(p => p.ports.length > 0 ||
        (p.rawText && /\b(departed|arrived|ARRIVES|DEPARTS|DUE:|loaded|unloaded)\b/i.test(p.rawText)));
  } catch(e) {
    return [];
  }
}
...
// ── 3. AIS Live Passages ──────────────────────────────────────────────────────
async function fetchOneAISPort(port, clientConfig) {
  try {
    const params = new URLSearchParams({
      port: port.id,
      sort: 'destination_eta',
      order: 'desc',
      offset: '0',
    });
    const url = `${clientConfig.apiBaseUrl}/passages?${params}`;
    const r = await fetchTimeout(url, {
      headers: {
        ...HEADERS,
        'X-API-Key': clientConfig.apiKey,
        Referer: `${BOATNERD_AIS_ORIGIN}/passage`,
      }
    }, 12000);
    ...
    if (text.trimStart().startsWith('<')) return { port: port.name, portId: port.id, vessels: [], status: 'unexpected_html' };
    let data;
    try { data = JSON.parse(text); } catch(e) { return { port: port.name, portId: port.id, vessels: [], status: 'invalid_json' }; }
    if (data.statusCode && data.statusCode !== 200) {
      return { port: port.name, portId: port.id, vessels: [], status: `API ${data.statusCode}` };
    }
    ...
    const vessels = rows.map(v => ({
      ...
      direction: v.direction_lbl || v.direction || v.bound || '',
      location: v.location_name || '',
      // destination_port_name is the BoatNerd watch point this list tracks, not
      // where the ship is going; destination is the crew-entered AIS field.
      watchPoint: v.destination_port_name || '',
      destination: v.destination || '',
      eta: v.destination_eta || v.eta || '',
      cargo: v.cargo_name || v.other_cargo || v.cargo || '',
      // gps_time is the position report. datein is a database insert time and
      // is not a position report, so it is never substituted for one.
      timestamp: v.gps_time || v.report_timestamp || v.timestamp || '',
      port: port.name,
    })).filter(v => v.name && v.name.length > 2);
    return { port: port.name, portId: port.id, vessels, total: data.data?.total || data.total || rows.length, status: 'ok', fetched_at: new Date().toISOString() };
  } catch(e) {
...
export async function scrapeAISPassages() {
  try {
    const clientConfig = await getBoatNerdClientConfig();
    return Promise.all(AIS_PORTS.map(port => fetchOneAISPort(port, clientConfig)));
  } catch (error) {
...
// ── 5. NOAA Water Levels ──────────────────────────────────────────────────────
// Uses water_level product (6-min readings) - daily_mean is not offered at Great Lakes stations
// datum=LWD = Great Lakes Low Water Datum (nautical chart datum, per IGLD 1985)
// Fetch yesterday's date - always fully available
async function fetchOneWaterLevel(station) {
  try {
    // Latest observation in UTC; the edition renders it in Eastern time, so
    // stations in different local time zones are never compared by raw clock.
    const params = new URLSearchParams({
      station:     station.id,
      product:     'water_level',
      datum:       'LWD',
    ...
    if (!Number.isFinite(level)) throw new Error('Non-numeric reading');
    const name = String(data.metadata?.name || '').trim();
    if (!name) throw new Error('NOAA returned no station name');
    return {
      lake: station.lake,
      city: name,
      stationName: name,
      ...
      level_ft: level,
      valueText: String(obs.v).trim(),
      observedAt: `${obs.t.replace(' ', 'T')}:00Z`,
      date: obs.t,
      preliminary: obs.q === 'p',
      status: 'ok',
    };
  } catch(e) {
    return { lake: station.lake, city: '', stationId: station.id, level_ft: null, status: 'error', error: e.message };
  }
}
...
async function fetchOneGLF(lake) {
  try {
    ...
    const synopsis = synMatch ? synMatch[1].replace(/\s+/g, ' ').trim().slice(0, 1200) : '';
