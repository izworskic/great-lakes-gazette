...
function openingSentence(value, maxLength = 600) {
  ...
  const maskPeriod = match => match.replaceAll('.', '\u0000');
  const masked = text
    .replace(/\b(?:a\.m|p\.m)\./gi, maskPeriod)
    .replace(/\b(?:St|Ste|Co|Corp|Inc|Ltd|Dr|Mr|Mrs|Ms|Capt|Lt|Mt|Ft|No|Mich|Wis|Ont|Ind|Ill)\./gi, maskPeriod)
    .replace(/\b(?:[A-Z]\.){1,4}/g, maskPeriod);
  const ending = masked.match(/[.!?](?:["')\]]*)?(?:\s|$)/);
  if (!ending || ending.index == null) return text;
  return text.slice(0, ending.index + ending[0].trimEnd().length);
}
...
// Lake and waterway sections use the news peg, not background later in the
// lead. This prevents a Sturgeon Bay repair story from becoming a Lake
// Superior story merely because it mentions an earlier stop in Superior.
export function issuePrimaryGeographyText(issue) {
  const brief = issue && typeof issue.brief === 'object' ? issue.brief : {};
  const sourceSections = Array.isArray(brief.sections) ? brief.sections.filter(section => section?.body) : [];
  const leadSection = sourceSections.find(section => !cleanText(section.kicker));
  return [
    brief.headline,
    brief.leadSubject,
    openingSentence(leadSection?.body || brief.brief),
  ].map(cleanText).filter(Boolean).join(' ');
}
...
function placeUsedAsLocation(text, place) {
  ...
  return new RegExp(
    `(?:(?<!far\\s)\\b(?:${locationPreposition})\\s+(?:the\\s+)?${name}\\b|` +
...
export function topicsForIssue(issue) {
  ...
  const geographyText = issuePrimaryGeographyText(issue);
  return TOPICS.filter(topic =>
    topic.patterns.some(pattern => pattern.test(
      topic.slug === 'great-lakes-freighters' || topic.slug === 'water-levels-marine-weather'
        ? leadStoryText
        : geographyText
    )) ||
    topic.places?.some(place => placeUsedAsLocation(geographyText, place))
  );
}
